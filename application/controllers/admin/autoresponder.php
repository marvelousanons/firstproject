...
class Autoresponder extends FSD_Controller 
{
	...
	var $module_name = 'Email Templates Manager';
	...
	{
		$json = $this->autoresponder_model->get_datatable($this->access);
		$raw = json_decode($json, true);
		if (isset($raw['data']) && is_array($raw['data'])) {
			foreach ($raw['data'] as &$row) {
				$row['delete'] =
					'<a href="'.site_url('admin/autoresponder/edit/'.$row['ID']).'" class="btn btn-warning btn-sm"><i class="fa fa-edit"></i></a>';
				$row['delete'] .=
					' <a href="'.site_url('admin/autoresponder/delete/'.$row['ID']).'" class="btn btn-danger btn-sm" onclick="return confirm(\'Delete this template?\')"><i class="fa fa-trash"></i></a>';
			}
		}
